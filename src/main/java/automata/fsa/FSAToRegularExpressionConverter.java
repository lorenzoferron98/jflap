...
public class FSAToRegularExpressionConverter {
     ...
     *
     * @param state       the state to remove.
     * @param transitions the transitions returned for removing <CODE>state</CODE>.
     * @param automaton   the automaton.
     */
     ...
     *
     * @param fromState   the from state for <CODE>transitions</CODE> and for the
     *                    newly created transition.
     * @param toState     the to state for <CODE>transitions</CODE> and for the newly
     *                    created transition.
     * @param transitions the transitions being removed and combined into a single
     *                    transition
     * @param automaton   the automaton
    ...
    public static FSATransition combineToSingleTransition(State fromState,
        ...
        String label = transitions[0].getDescription();
        automaton.removeTransition(transitions[0]);
        for (int i = 1; i < transitions.length; i++) {
            label = or(label, transitions[i].getDescription());
     ...
     * @param ij the expression on the arc from the initial state to the final
     *           state.
     * @param jj the expression on the loop off the final state.
     * @param ji the expression on the arc from the final state to the initial
     *           state.
     ...
     *
     * @param automaton a generalized transition graph with only two states, a unique
     ...
     *
     * @param automaton the generalized transition graph with two states (a unique
     *                  initial and final state).
