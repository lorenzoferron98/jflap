...
public class PDAToCFGConverter {
    ...
    public boolean hasSingleFinalState(Automaton automaton) {
        ...
        for (int k = 0; k < transitions.length; k++) {
            ...
            if (!(toPop).endsWith(BOTTOM_OF_STACK)) {
    ...
    public boolean isInCorrectFormForConversion(Automaton automaton) {
        return hasSingleFinalState(automaton)
     ...
     *
     * @param lhs              the variable on the left-hand side.  The initial variable is the initial state + 'Z' +
     *                         the final state.
     * @param productions      the current list of productions.
     * @param valid            set of variables that potentially end in terminals
     * @param validProductions an int array used to mark all productions that should not be removed.  The values that
     ...
     * @param model     the table of productions
     * @author Chris Morgan
     */
