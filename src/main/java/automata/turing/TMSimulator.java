...
public class TMSimulator extends AutomatonSimulator {
     ...
     * @throws IllegalArgumentException if this automaton is not a Turing machine
     */
...
//       //END MERLIN MERLIN MERLIN MERLIN MERLIN// //this code is only for show, it should be moved into a setting with a better UI before release//
    ...
    public Configuration[] getInitialConfigurations(String[] inputs) {
        ...
        for (int i = 0; i < tapes.length; i++) {
            if (Universe.curProfile.getSudkampConvenctionsEnabled())
                tapes[i] = new Tape(Tape.BLANK + inputs[i]);
            else
...
//                }
        ...
        if (assignIndex == -1 && bangIndex == -1) { //ordinary case
            ...
            for (String character : characters) {
                ...
                if (character.charAt(0) == underHead) flag = true;
    ...
    public ArrayList<Configuration> stepConfiguration(Configuration config) { //one step, and will dig into building blocks if necessary
        ...
        TuringMachine tmp; //just a literally tmp, like /tmp
        ...
        while (true) {
            ...
            Arrays.sort(trans, (a, b) -> { //variables are only allowed with SINGLE TAPE, and same with NOT
     ...
     *
     * @param input the input string to be run on the automaton
    ...
    public boolean simulateInput(String input) {
        ...
        for (Configuration initialConfig : initialConfigs) {
            TMConfiguration initialConfiguration = (TMConfiguration) initialConfig;
