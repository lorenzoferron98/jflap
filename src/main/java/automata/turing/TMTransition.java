...
public class TMTransition extends Transition {
     ...
     * @param ntoRead    the string that the machine should satisfy before moving on to
     *                   the next state
     * @param ntoWrite   the string that the machine should write on to the tape
     ...
     * @param from           the state this transition comes from
     * @param to             the state this transition goes to
     * @param toReadArray    the strings that the machine should satisfy in each tape
     *                       before moving on to the next state
     * @param toWriteArray   the strings that the machine should write on each tape
     * @param directionArray the direction to move the read/write tape head on each tape
     * @throws IllegalArgumentException if the number of elements in each array is not the same, or
    ...
    public Transition copy(State from, State to) {
        ...
        return new TMTransition(from, to, toRead.toArray(s),
                toWrite.toArray(s), direction.toArray(s));
    }

    /**
     * Returns the input to read for the given tape.
     *
     * @param tape the tape index to retrieve
     */
     ...
     * this transition.
     *
     * @param tape the tape to return the
     */
     ...
     *
     * @param tape the tape index which this direction will affect
     ...
     *
     * @param newDirection the new direction to move the tape head
     * @param tape         the tape index to change the direction for, indexed 0 through
