...
public abstract class AutomatonTransducer extends AbstractTransducer {
    ...
    private final Map<String, Automaton> automatonMap = new java.util.HashMap<>();
     ...
     * @throws DataException if the state ID is not in a supported format
     */
     ...
     * @param locatedStates if not <CODE>null</CODE>, this set will be filled with
     ...
     * @throws DataException in the case of non-numeric, negative, or duplicate IDs
     * @see #readTransitions
     */
    ...
    protected void createState(ArrayList<Node> stateNodes, Map<Integer, Node> i2sn,
        ...
        while (it.hasNext()) {
            ...
            double x = 0, y = 0;
            try {
                x = Double.parseDouble(e2t.get(STATE_X_COORD_NAME));
            ...
            try {
                y = Double.parseDouble(e2t.get(STATE_Y_COORD_NAME));
            } catch (NullPointerException e) {
                hasLocation = false;
            } catch (NumberFormatException e) {
...
//			if (!isBlock){
            if (!(automaton instanceof TuringMachine)) {
                ...
                if (e2t.containsKey(FILE_NAME)) {
                    String fileName = e2t.get(FILE_NAME);
...
//                    EDebug.print("Are we or not creating a block?");
     ...
     * @param map       the map of ID objects to a state
     ...
     * @param locStates the states that have the x and y tags in the DOM
