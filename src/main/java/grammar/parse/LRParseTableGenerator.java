...
/**
 * This class generates {@link LRParseTable}s. The intention is
...
public abstract class LRParseTableGenerator {
    ...
    public static LRParseTable generate(Grammar grammar,
        ...
        for (int i = 0; i < finals.length; i++) {
            Set<Production> items = stateToItems.get(finals[i]);
            ...
            while (it.hasNext()) {
                ...
                if (p.getRHS().endsWith("" + Operations.ITEM_POSITION)) {
                    ...
                    while (!p2.equals(ps[j]))
                        j++;
                    Set<String> follow = followSets.get(p.getLHS());
                    Iterator<String> fit = follow.iterator();
                    while (fit.hasNext()) {
                        String followSymbol = fit.next();
