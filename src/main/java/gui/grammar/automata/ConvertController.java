...
public abstract class ConvertController {
     ...
     * @param automaton the automaton to build the <CODE>ConvertController</CODE>
    ...
    protected void fillMap() {
        ...
        for (State state : states) {
            Production[] prods = getProductions(state);
            if (prods.length == 0)
                continue;
            objectToProduction.put(state, prods);
            IntStream.range(0, prods.length).forEach(j -> productionToObject.put(prods[j], state));
        }
        // Now let's get the other cannon!
        Transition[] transitions = automaton.getTransitions();
        for (Transition transition : transitions) {
            Production[] prods = getProductions(transition);
            if (prods.length == 0)
                continue;
            objectToProduction.put(transition, prods);
            IntStream.range(0, prods.length).forEach(j -> productionToObject.put(prods[j], transition));
        }
    }
    ...
    public Object revealRandomProductions() {
        for (Entry<Serializable, Production[]> entry : objectToProduction.entrySet()) {
    ...
    public Object[] highlightUntransformed() {
        ...
        drawer.clearSelected();
        IntStream.range(0, unselected.length).forEach(i -> {
     ...
     * @throws GrammarCreationException if there is some impediment to the creation of the grammar
     */
