...
public class FSAConvertController extends ConvertController {
     ...
     * @param automaton the automaton to build the <CODE>FSAConvertController</CODE>
    ...
    public FSAConvertController(ConvertPane pane, SelectionDrawer drawer,
        ...
        IntStream.range(0, states.length).forEach(i -> states[i].setLabel(converter.variableForState(states[i])));
    }
    ...
    protected Production[] getProductions(State state) {
        ...
        return new Production[]{converter.getLambdaProductionForFinalState(
                getAutomaton(), state)};
    }
    ...
    protected Production[] getProductions(Transition transition) {
        return new Production[]{converter.getProductionForTransition(transition)};
    }
    ...
    protected Grammar getGrammar() {
        ...
        ArrayList<Production> productions = IntStream.range(0, rows).mapToObj(i -> getModel().getProduction(i)).filter(Objects::nonNull).sorted(new Comparator<Object>() {
