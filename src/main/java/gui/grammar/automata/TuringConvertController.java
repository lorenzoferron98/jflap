...
*/
    ...
    protected ConvertedUnrestrictedGrammar getGrammar() {
        ...
        ArrayList<Production> productions = IntStream.range(0, rows).mapToObj(i -> getModel().getProduction(i)).filter(Objects::nonNull).sorted(new ProductionComp()).collect(Collectors.toCollection(ArrayList::new));
...
//	}
    ...
    class ProductionComp implements Comparator<Production> {
        public int compare(Production p1, Production p2) {
            if ("S".equals(p1.getLHS())) return p1.getLHS().equals(p2.getLHS()) ? 0 : 1;
