...
class ConvertController {
     ...
     * @param productionsToTransitions a map from productions to the corresponding transitions the
     *                                 user should come up with... this maping must be one-to-one
     * @param parent                   some parent object so that the controller knows where to put
    ...
    private Map<Transition, Production> invert(Map<Production, Transition> map) {
        ...
        try {
            ...
            while (it.hasNext()) {
                Entry<Production, Transition> entry = it.next();
    ...
    private void initListeners() {
        ...
        grammarView.addSelectionListener(event -> {
            ...
            Arrays.stream(p).forEach(production -> drawer.addSelected(pToT.get(production)));
            parent.repaint();
        });
    }
    ...
    public void complete() {
        ...
        productions.stream().filter(p -> !alreadyDone.contains(p)).map(p -> pToT.get(p)).forEach(t -> automaton.addTransition(t));
    }
    ...
    public void createForSelected() {
        Production[] p = grammarView.getSelected();
        IntStream.range(0, p.length).filter(i -> !alreadyDone.contains(p[i])).mapToObj(i -> pToT.get(p[i])).forEach(t -> automaton.addTransition(t));
    }
