...
public class GrammarViewer extends JTable {
    ...
    public GrammarViewer(Grammar grammar) {
        ...
        IntStream.range(0, prods.length).forEach(i -> {
            data[i][0] = prods[i];
            data[i][1] = Boolean.FALSE;
            productionToRow.put(prods[i], i);
        });
    ...
    public Production[] getSelected() {
        ...
        return Arrays.stream(rows).mapToObj(row -> (Production) data[row][0]).toArray(Production[]::new);
    }
     ...
     * @param checked    <CODE>true</CODE> if the production should be marked as
