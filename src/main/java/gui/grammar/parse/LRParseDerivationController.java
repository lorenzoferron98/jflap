...
public class LRParseDerivationController extends LLParseDerivationController {
     ...
     */
    private final ItemSetChooser itemChooser;
    /**
     * The augmented grammar.
     */
    private final Grammar augmented;
    /**
     * The parse table derivation pane.
     */
    private final LRParseTableDerivationPane derivation;
    /**
     * The mapping of states to a set of items.
     */
    private final Map<State, Set<Production>> stateToItems = new HashMap<>();
    /**
     * The mapping of item sets to a state.
     */
    private final Map<Set<Production>, State> itemsToState = new HashMap<>();
    /**
     * The editor.
     */
     ...
     * @param derivation  the view component with the user interface elements where the
    ...
    boolean done() {
        switch (step) {
            ...
            case BUILD_DFA:
                ...
                int selected = 0;
                while (it.hasNext()) {
                    Entry<Set<Production>, State> entry = it.next();
     ...
     *
     * @param first  the state that represents the group of items goto takes as an
     *               argument
     * @param point  if a new state need be created, this will be the point that
     *               state is created at
     * @param second the state that represents the group of items goto evaluates
    ...
    public void gotoGroup(State first, Point point, State second) {
        ...
        if (second != null) {
            ...
            Set<Production> toUser = stateToItems.get(second);
            if (!to.equals(toUser)) {
