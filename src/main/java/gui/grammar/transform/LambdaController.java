...
public class LambdaController {
            ...
            case PRODUCTION_MODIFY:
                ...
                for (Production production : p) {
                    ...
                    if (production.getRHS().length() == 0) {
                        lambdaProductions.add(production);
                        continue;
                    }
                    Production[] p2 = remover.getProductionsToAddForProduction(
                            production, lambdaVariables);
                    desiredProductions.add(production);
                    productionsToExpansion.put(production, p2);
                    IntStream.range(0, p2.length).forEach(j -> desiredProductions.add(p2[j]));
                }
    ...
    public void expandRowProduction(int row) {
        ...
        IntStream.range(0, ps.length).forEach(i -> {
    ...
    public void doStep() {
        switch (step) {
            ...
            case PRODUCTION_MODIFY:
                ...
                IntStream.range(0, m.getRowCount() - 1).forEach(this::expandRowProduction);
                nextStep();
                break;
            case FINISHED:
                break;
