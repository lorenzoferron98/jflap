...
public class UnitController {
    ...
    private void nextStep() {
        ...
        switch (step) {
            case VARAIBLE_GRAPH: {
                ...
                IntStream.range(0, p.length).filter(i -> ProductionChecker.isUnitProduction(p[i])).forEach(i -> vdgTransitions.add(remover.getTransitionForUnitProduction(
                        p[i], vdg)));
            ...
            case PRODUCTION_MODIFY: {
                ...
                Production[] p = grammar.getProductions();
                Arrays.stream(p).forEach(production -> {
                    pane.editingGrammarModel.addProduction(production);
                ...
                Collections.addAll(desiredProductions, p);
                updateDisplay();
    ...
    void doSelected() {
        ...
        selectedUnitProductions.forEach(unit -> {
            // Determine if this desired production is good to go.
            desiredProductions.stream().filter(p -> p.getLHS().equals(unit.getRHS())).map(p -> new Production(unit.getLHS(), p.getRHS())).forEach(toAdd::add);
        });

        // Add those productions!
        toAdd.stream().filter(p -> currentProductions.add(p)).forEach(p -> pane.editingGrammarModel.addProduction(p));
        pane.editingActive = true;
    ...
    public void doStep() {
        switch (step) {
            case VARAIBLE_GRAPH:
                ...
                Arrays.stream(t).forEach(transition -> vdg.addTransition(transition));
                break;
            case PRODUCTION_MODIFY:
                ...
                IntStream.range(0, p.length).filter(i -> currentProductions.add(p[i])).forEach(i -> pane.editingGrammarModel.addProduction(p[i]));
                nextStep();
                break;
            case FINISHED:
                break;
    ...
    void stateClicked(State state, MouseEvent event) {
        ...
        } else if (state == null) pane.vdgDrawer.clearSelected();
        else if (!pane.vdgDrawer.isSelected(state)) {
    ...
    boolean productionDeleted(Production production, int row) {
        if (!ProductionChecker.isUnitProduction(production)) return false;
