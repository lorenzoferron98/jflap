...
class BuilderController {
     ...
     * @param minDfa    the DFA being built; this should initially have just the
     *                  states and nothing else
     * @param drawer    the selection drawer for the DFA being built
     * @param minimizer the minimizer object handling the grit of the minimization
     *                  process
    ...
    private void determineTransitions() {
        ...
        IntStream.range(0, states.length).forEach(i -> remainingTransitions.addAll(minimizer.getTransitionsForState(
    ...
    private void initTransitionListener() {
        minDfa.addTransitionListener(e -> {
            ...
            } else remainingTransitions.remove(transition);
        });
    }
    ...
    public void complete() {
        ...
        new HashSet<>(remainingTransitions).stream().map(o -> (Transition) o).forEach(minDfa::addTransition);
    }
