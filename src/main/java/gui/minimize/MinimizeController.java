...
class MinimizeController {
     ...
     *
     * @param node  the node that was under the mouse
     * @param event the corresponding mouse event
     */
    ...
    public void toggleState(MinimizeTreeNode node, State state) {
        ...
        try {
            ...
            for (TreeNode treeNode : children) {
                MinimizeTreeNode child = (MinimizeTreeNode) treeNode;
                if (child == node)
                    continue;
     ...
     *
     * @param node the node to split, which must already have a terminal on it
    ...
    private void split(MinimizeTreeNode node) {
        ...
        groups.forEach(group -> addChild(node, group));
        expanding = null;
    }
     ...
     *
     * @param root the root of the subtree to split completely
     */
    public void splitSubtree(MinimizeTreeNode root) {
        ...
        Arrays.stream(children).map(child -> (MinimizeTreeNode) child).forEach(this::splitSubtree);
    }
     ...
     * the given node.
     *
     * @param node the node to get the states from
     */
    private void setSelectedStates(MinimizeTreeNode node) {
        automatonDrawer.clearSelected();
        State[] states = node.getStates();
        Arrays.stream(states).forEach(automatonDrawer::addSelected);
        treeDrawer.clearSelected();
     ...
     *
     * @param node the node whose children must be destroyed
    ...
    private MinimizeTreeNode[] killChildren(MinimizeTreeNode node) {
        ...
        IntStream.range(0, children.length).forEach(i -> {
    ...
    private void addChildren(MinimizeTreeNode parent,
        ...
        IntStream.range(0, children.length).forEach(i -> getTree().insertNodeInto(children[i], parent,
    ...
    public boolean check(MinimizeTreeNode node) {
        ...
        for (MinimizeTreeNode child : children) {
        ...
        // Remove any children that may exist.
        HashSet<HashSet<State>> realPartitions;
        ...
        realPartitions = groups.stream().map(group -> new HashSet<>(Arrays.asList(group))).collect(Collectors.toCollection(HashSet::new));
     ...
     *
     * @param node the node we may expand
     ...
     * displayed.
     *
     * @param parent the node that holds the group we're modifying partitions of,
