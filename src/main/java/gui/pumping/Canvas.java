...
/**
 * A <code>Canvas</code> is an area where strings, {@link Text},
 * are animated as {@link MovingText} and will move
 * to construct the pumped string.
 *
 * @author Jinghui Lim
 * @see PumpingLemmaInputPane
 */
public class Canvas extends JPanel {
    ...
    public Text addText(String s) {
        Point2D p;
        if (myInitialText.isEmpty()) p = FIRST_ROW;
        else {
    ...
    public void moveText(int[] n) {
        ...
        for (int i = 0; i < myInitialText.size(); i++) {
            ...
            for (int j = 0; j < n[i]; j++) {
                ...
                p = myMovingText.isEmpty() ? SECOND_ROW : new Point2D.Double(SECOND_ROW.x + distance, SECOND_ROW.y);
    ...
    public void paintComponent(Graphics pen) {
        myLabelText.forEach(l -> l.paint(pen));
        myInitialText.forEach(t -> t.paint(pen));

        myFinalText.forEach(t -> t.paint(pen));
