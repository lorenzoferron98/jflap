...
public abstract class PumpingLemmaInputPane extends JPanel {
     ...
     */
    private static final String PROMPT_ANIM = "Click \"Step\" in Box 5 to step the animation.";
    /**
     * The instruction that prompts the user to view the animation or add the case.
     */
    private static final String PROMPT_CASE = "Click \"Step\" in Box 5 to step the animation or " +
    ...
    private void initLeftPanel() {
        ...
        IntStream.range(0, stages.length).forEach(i -> leftPanel.add(stages[i]));
        if (this instanceof HumanFirstPane)
            stages[2].setVisible(false);
        IntStream.range(3, stages.length).forEach(i -> stages[i].setVisible(false));
    }

    /**
     * Initializes and returns the right panel, a {@link CasePanel}
    ...
    protected void resetMessages() {
        Arrays.stream(stageMessages).forEach(stageMessage -> stageMessage.setText(""));
    }
    ...
    protected void reset() {
        ...
        setVisibilityStages(this instanceof HumanFirstPane ? 2 : 3, false);
        leftPanel.revalidate();
    }
    ...
    public void updateTopPane(boolean printExplanation) {
        ...
        String a;
        String output;
        s = new StringBuilder("<b>My Attempts:</b>");
        output = "<html>";
        ...
        for (int i = myLemma.getAttempts().size() - 1; i >= 0; i--) {
            ...
            s.append("<br>").append(i + 1).append(":  ").append(a);
        }
