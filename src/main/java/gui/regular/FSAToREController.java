...
public class FSAToREController {
     ...
     * @param drawer     the selection drawer in the editor
     * @param mainStep   the label holding the description of the main step
     * @param detailStep the label holding the detail description of whatever the user
     *                   must do now
    ...
    protected int collapsesNeeded() {
        ...
        for (State value : states)
            for (State state : states)
                if (automaton.getTransitionsFromStateToState(value,
                        state).length > 1)
                    needed++;
        return needed;
    }
     ...
     *
     * @param point the point that the state creation tool was clicked at
    ...
    public State stateCreate(Point point) {
        ...
        drawer.clearSelected();
        Arrays.stream(finals).forEach(aFinal -> {
            automaton.removeFinalState(aFinal);
            drawer.addSelected(aFinal);
        });
    ...
    public Transition transitionCreate(State from, State to) {
        if (currentStep == TRANSITIONS_TO_SINGLE_FINAL) {
            ...
            frame.repaint();
            if (drawer.numberSelected() == 0) nextStep();
            return t;
        }
     ...
     * about it.
     *
     * @param transition the transition that was selected, or <CODE>null</CODE> if
     *                   less or more than one transition is selected
     */
    public void tableTransitionSelected(Transition transition) {
        drawer.clearSelectedTransitions();
        if (transition == null || collapseState == null) return;
    ...
    public void moveNextStep() {
        switch (currentStep) {
            ...
            case TRANSITIONS_TO_SINGLE_FINAL:
                ...
                Arrays.stream(states).forEach(state -> transitionCreate(state, finalState));
                break;
            case CONVERT_TRANSITIONS: {
                State[] s = automaton.getStates();
                Arrays.stream(s).forEach(state -> Arrays.stream(s).filter(value -> automaton.getTransitionsFromStateToState(state, value).length > 1).forEach(value -> transitionCollapse(state, value)));
                break;
            }
            case CREATE_EMPTY_TRANSITIONS: {
                State[] s = automaton.getStates();
                Arrays.stream(s).forEach(value -> Arrays.stream(s).filter(state -> automaton.getTransitionsFromStateToState(value, state).length == 0).forEach(state -> transitionCreate(value, state)));
                break;
            }
            case COLLAPSE_STATES:
                State[] s = automaton.getStates();
                Arrays.stream(s).filter(state -> automaton.getFinalStates()[0] != state
                        && automaton.getInitialState() != state).forEach(state -> {
                    Transition[] t = FSAToRegularExpressionConverter.getTransitionsForRemoveState(state,
                            automaton);
                    FSAToRegularExpressionConverter.removeState(state, t, automaton);
                });
                remaining = 0;
                nextStep();
                break;
