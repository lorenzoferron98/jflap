...
public class REToFSAController {
     ...
     */
    private final Set<FSATransition> toDoTransitions = new HashSet<>();
    /**
     ...
     *
     * @param expression the expression to check for actions that may be required
     */
     ...
     * @param exps       the array of string expressions to replace the transition with
    ...
    public void completeStep() {
        ...
        switch (action) {
            case DEPARENS:
                ...
                return;
            case DEOR:
                Arrays.stream(replacements).forEach(replacement -> {
                    automaton.addTransition(lambda(from, replacement
                            .getFromState()));
                    automaton
                            .addTransition(lambda(replacement.getToState(), to));
                });
                break;
            case DECAT:
                automaton
        ...
        switch (action) {
            ...
            case DEOR:
                if (from == transition.getFromState())
                    valid = Arrays.stream(replacements).anyMatch(replacement -> replacement.getFromState() == to);
                if (to == transition.getToState())
                    if (Arrays.stream(replacements).anyMatch(replacement -> replacement.getToState() == from))
                        valid = true;
                break;
            case DECAT:
    ...
    private void nextStep() {
        if (transitionNeeded == 0) {
            if (toDo.size() > 0) {
                convertPane.mainLabel.setText(action != 0 ? "Resolution complete." : "Welcome to the converter.");
