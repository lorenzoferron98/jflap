...
public class ConfigurationController implements ConfigurationSelectionListener {
    ...
    */
     ...
     * @param simulator the automaton simulator
     * @param drawer    the drawer of the automaton
     * @param component the component in which the automaton is displayed
     */
    ...
    public void reset() {
        ...
        if (simulator instanceof TMSimulator) {
            ...
            Configuration[] configs = tmSim.getInitialConfigurations(tmSim
                    .getInputStrings());
            Arrays.stream(configs).forEach(configurations::add);
        } else IntStream.range(0, originalConfigurations.length).forEach(i -> {
    ...
    public void cleanup() {
        ...
        for (TraceWindow window : windows) window.dispose();
    ...
    public void step(boolean blockStep) {
        ...
        if (!blockStep) { //for ordinary automaton
            ...
            IntStream.range(0, configs.length).forEach(i -> {
...
//  }
    ...
    public void freeze() {
        ...
        Arrays.stream(configs).forEach(configurations::setFrozen);
    ...
    public void remove() {
        ...
        Arrays.stream(configs).forEach(configurations::remove);
        configurations.validate();
        configurations.repaint();
    }
...
//				drawer.setAutomaton(setWith);
    ...
    public void defocus() {
        ...
        IntStream.range(0, configs.length).filter(i -> configs[i].getFocused()).mapToObj(i -> configs[i]).forEach(configurations::defocus);
    ...
    public void thaw() {
        ...
        Arrays.stream(configs).forEach(configurations::setNormal);
        configurations.deselectAll();
        configurations.repaint();
    }
    ...
    public void trace() {
        ...
        IntStream.range(0, configs.length).forEach(i -> {
            TraceWindow window = configurationToTraceWindow
                    .get(configs[i]);
            if (window == null) configurationToTraceWindow.put(configs[i], new TraceWindow(
                    configs[i]));
            else {
