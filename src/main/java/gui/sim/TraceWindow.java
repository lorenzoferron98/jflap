...
public class TraceWindow extends JFrame {
     ...
     *
     * @param last the last configuration that we are tracing; we display it
     ...
     *
     * @param configuration the configuration whose ancestry we want to display
    ...
    public static JScrollPane getPastPane(Configuration configuration) {
        ...
        sp.validate();
        if (sp.getSize().height > MAXHEIGHT) sp.setSize(sp.getSize().width, MAXHEIGHT);
        return sp;
    }
