...
public class InputTableModel extends GrowableTableModel {
    ...
    protected final static TableModelListener LISTENER = new TableModelListener() {
        public void tableChanged(TableModelEvent event) {
            ...
            Integer inputs = model.getInputCount();
    ...
    public static InputTableModel getModel(Automaton automaton, boolean multipleFile) {
        InputTableModel model = INPUTS_TO_MODELS
        ...
        if (multipleFile) model.isMultiple = true;
        return model;
    }
    ...
    public String getColumnName(int column) {
        ...
        String word = "";
        if (column <= (getInputCount() - 1 + offset) && column >= (offset)) word = "Input";
     ...
     * @param config the associated configuration, or <CODE>null</CODE> if you
    ...
    public void setResult(int row, String result, Configuration config, ArrayList<String> comparison, int index) {
        ...
        if (config instanceof TMConfiguration && config != null) {
            ...
            outNum = tapes.length;
            if (config.isAccept()) for (int i = 0; i < tapes.length; i++) {
    ...
    public Configuration getAssociatedConfigurationForRow(int row) {
        return rowToAssociatedConfiguration
                .get(row);
    }
}
