...
public class DefaultTreeDrawer implements TreeDrawer, TreeModelListener {
     ...
     *
     * @param tree the tree to draw, assumed to have all nodes as <CODE>javax.swing.tree.TreeNode</CODE>
     *             objects.
     * @see TreeNode
     */
     ...
     *
     * @param point a point in ([0,1], [0,1])
     * @param size  some dimensions
     ...
     * @param size   the dimension of the area the graph is drawn in
     * @param parent the point that the parent was drawn at, or <CODE>null</CODE>
    ...
    protected void draw(Graphics2D g, TreeNode node, Dimension2D size,
        ...
        Arrays.stream(c).forEach(treeNode -> draw(g, treeNode, size, visible ? p : null));
     ...
     * Shows a node.
     *
     * @param node the node to set as visible
     */
     ...
     * Hides a node.
     *
     * @param node the node to set as invisible
     */
     ...
     *
     * @param node the node to get visibility of
    ...
    public TreeNode nodeAtPoint(Point2D point, Dimension2D size) {
        for (Entry<TreeNode, Float> entry : nodeToPoint.entrySet()) {
    ...
    private void setPoints(TreeNode node, int depth, int thisDepth,
        ...
        nodeToPoint.put(node, new Float(x, y));
        // Update the depth and width figures.
        widthSofar[thisDepth++]++;
        // Recurse on children.
        TreeNode[] children = Trees.children(node);
        for (TreeNode child : children) setPoints(child, depth, thisDepth, width, widthSofar);
    }
     ...
     *
     * @param inverted <CODE>true</CODE> if the root should be at the base, <CODE>false</CODE>
