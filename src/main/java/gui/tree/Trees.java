...
public class Trees {
    ...
    private static void width(TreeNode node, int depth, int[] array) {
        ...
        for (TreeNode child : children) width(child, depth, array);
    }
    ...
    public static int depth(TreeNode node) {
        TreeNode[] children = Trees.children(node);
        int max = -1;
        for (TreeNode child : children) max = Math.max(max, depth(child));
        return max + 1;
    }
    ...
    public static TreeNode[] leaves(TreeNode node) {
        ...
        return Arrays.stream(children).map(Trees::leaves).flatMap(Arrays::stream).toArray(TreeNode[]::new);
    }
}
