...
public class AutomatonDrawer {
    ...
    public AutomatonDrawer(Automaton automaton) {
         ...
         * state.
         */
        if (automaton instanceof automata.mealy.MooreMachine)
...
//			drawState(g, hs.get(i));
        ...
        State[] states = automaton.getStates();
        IntStream.range(0, states.length).forEach(i -> drawState(g, states[i]));
...
//			System.out.println("Using cache");
        ...
        notes.stream().map(curNote -> new Rectangle(curNote.getAutoPoint(), new Dimension(curNote.getBounds().getSize()))).forEach(rect::add);
        arrowToTransitionMap.keySet().stream().map(CurvedArrow::getBounds).forEach(rect::add);
        validBounds = true;
//		return cachedBounds = rect;
    ...
    protected void drawState(Graphics g, State state) {
        ...
        if (drawLabels) statedrawer.drawStateLabel(g, state, state.getPoint(),
    ...
    public Transition transitionAtPoint(Point point) {
        if (!valid)
            refreshArrowMap();
        Set<CurvedArrow> arrows = arrowToTransitionMap.keySet();
        return arrows.stream().filter(arrow -> arrow.isNear(point, 2)).findFirst().map(arrow -> arrowToTransitionMap.get(arrow)).orElse(null);
    }
     ...
     *
     * @param drawLabels <CODE>true</CODE> if state labels should be drawn in the
