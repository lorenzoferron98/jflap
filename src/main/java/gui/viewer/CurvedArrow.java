...
public class CurvedArrow {
     ...
     */
    private static final double ARROW_ANGLE = Math.PI / 10;
    /**
     ...
     */
    private static final AffineTransform AFFINE_TURN_180;
    /**
     ...
     **/
    public static Color ARROW_COLOR = new Color(0, 0, 0);
    /**
     * A graphics object.
     */
     ...
     * The stored bounds.
     */
    protected Rectangle2D bounds = new java.awt.Rectangle(0, 0);
    /**
     ...
     * @param curvy the curvi-ness factor; 0 will create a straight line; 1 and -1
     *              are rather curvy
     */
     ...
     *
     * @param curvy the new curvy factor
     */
     ...
     *
     * @param label the new label for the arrow
     */
     ...
     * @param head the point that is the point of the head of the arrow
     * @param away the point opposite from where the arrow is pointing, a point
    ...
    private void drawArrow(Graphics g, Point head, Point away) {
        int endX, endY;
        double angle = Math.atan2(away.x - head.x,
                away.y - head.y);
        angle += ARROW_ANGLE;
...
//        System.out.println("Curve refreshing");
     ...
     * @param fudge the radius around the point that should be checked for the
     ...
     * @param fudge the "fudge" factor
     * @param c     the curve we're checking for intersection with
     * @return <TT>true</TT> if the point is on the curve within a certain
     * fudge factor, <TT>false</TT> otherwise
     */
