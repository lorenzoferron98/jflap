...
public class SelectionDrawer extends AutomatonDrawer {
     ...
     */
    private final Set<Serializable> selected = new HashSet<>();
    private final Set<Serializable> selectedTransitions = new HashSet<>();
    /**
     ...
     *
     * @param automaton the automaton to select
     */
    ...
    protected void drawTransitions(Graphics g) {
        ...
        selectedTransitions.stream().map(selectedTransition -> (Transition) selectedTransition).forEach(t -> {
            try {
    ...
    public State[] getSelected() {
        return selected.toArray(new State[0]);
    }

    /**
     * Determines if a particular state is selected.
     *
     * @param state the state to check for "selectedness"
     ...
     *
     * @param transition the transition to set as unselected
     */
     ...
     *
     * @param transition the transition to check for "selectedness"
