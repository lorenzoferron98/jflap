...
public abstract class PumpingLemma implements Serializable {
     ...
     */
    public void doAll() {
        IntStream.range(0, myAllCases.size()).filter(i -> !myDoneCases.contains(myAllCases.get(i))).forEach(i -> myDoneCases.add(myAllCases.get(i)));
    }
    ...
    public void clearDoneCases() {
        myDoneCases.clear();
        IntStream.range(0, myAllCases.size()).forEach(i -> ((Case) myAllCases.get(i)).reset());
    }
    ...
    public ArrayList<String> getDoneDescriptions() {
        return myDoneCases.stream().map(Object::toString).collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    /**
     * Returns the list of done {@link Case}s.
     *
