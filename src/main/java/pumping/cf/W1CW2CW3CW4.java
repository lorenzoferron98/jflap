...
public class W1CW2CW3CW4 extends ContextFreePumpingLemma {
        ...
        for (int i = s1.length() - m + 2; i < s1.length(); i++)
            for (int j = 0; j < i; j++)
                if (s1.charAt(i) == s1.charAt(j)) {
                    first = s1.substring(0, i) + pumpString("" + s1.charAt(i), 2) + s1.substring(i + 1);
                    last = s1.substring(0, j) + pumpString("" + s1.charAt(j), 2) + s1.substring(j + 1);
                    x = s1.length() - i + j;
    ...
    public boolean isInLang(String s) {
        ...
        return IntStream.range(0, 4).noneMatch(i -> LemmaMath.otherCharactersFound(w[i], list) || w[i].length() == 0) && (w[0].equals(w[1]) || w[2].equals(w[3]));
    }
}
